...
int main() {
  ...
  // vehicle's start lane number
  int lane = 1;

  // vehicle's reference velocity in mph
  double ref_vel = 0.0;
        ...
        if (event == "telemetry") {
          ...
          // Main car's localization Data
            ...
            for (int i=0; i<sensor_fusion.size(); i++)
            {
              ...
              // predict position of that car
              double vx = sensor_fusion[i][3];
              double vy = sensor_fusion[i][4];
              double check_speed = sqrt(vx*vx + vy*vy);
              check_car_s += prev_path_size * 0.02 * check_speed;
              int check_car_lane = (int)(check_car_d / 4.0);
            ...
            if (car_ahead[lane] < 30.0)
            {
              ...
              else if (lane == 1)
              {
                ...
                else if (left_lane_possible)
                {
                  lane = 0;
                }
                else if (right_lane_possible)
                {
                  lane = 2;
                }
              }
            ...
            // later we will interpolate these waypoints by a spline and fill it with more points
            vector<double> ptsx;
            vector<double> ptsy;

            // reference x, y and yaw state (starting point where the car is or end point of previous path)
            double ref_x = car_x;
            double ref_y = car_y;
            ...
            {
              // redefine reference point as previous path's end point
            ...
            // shift and rotate coordinates into car's coordinate system
            ...
            // calculate how to break up spline points so that we travel at our desired vehicle velocity
            ...
            for (int i=1; i<=50-prev_path_size; i++)
            {
              ...
              // shift and rotate back to world's coordinate system
